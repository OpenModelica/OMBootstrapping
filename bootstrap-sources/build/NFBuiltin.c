...
void omc_NFBuiltin_makeBuiltinLookupTree(threadData_t *threadData, modelica_string _name, modelica_metatype _components, modelica_metatype _classes)
{
...

  {
    ...
    for (tmpMeta1 = _components; !listEmpty(tmpMeta1); tmpMeta1=MMC_CDR(tmpMeta1))
    {
      ...
      tmpMeta2 = omc_mk_box2(4, &NFLookupTree_Entry_COMPONENT__desc, omc_mk_integer(_i));
...

  {
    ...
    for (tmpMeta4 = _classes; !listEmpty(tmpMeta4); tmpMeta4=MMC_CDR(tmpMeta4))
    {
      ...
      tmpMeta5 = omc_mk_box2(4, &NFLookupTree_Entry_COMPONENT__desc, omc_mk_integer(_i));
  ...
  tmp7 = stringAppend(_OMC_LIT0,_name);
  tmp8 = stringAppend(tmp7,_OMC_LIT1);
  fputs(omc_string_data(tmp8),stdout);

  fputs(omc_string_data(mmc_anyString(_ltree)),stdout);
